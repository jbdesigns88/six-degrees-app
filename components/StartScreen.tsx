...
import { UserProfile, GameMode, CpuDifficulty } from '../types';
...
const StartScreen: React.FC<StartScreenProps> = ({ onStartGame, onSwitchToLobby, playerRating }) => {
  ...
  return (
    <div className="flex flex-col items-center justify-center h-full text-center p-4 bg-gray-900 text-white">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-4 w-full max-w-xl">
        {/* Solo Practice */}
        <button onClick={onStartGame} className="p-6 bg-gray-800 rounded-lg shadow-lg text-left transition-transform transform hover:scale-105 hover:shadow-cyan-500/30 focus:outline-none focus:ring-2 focus:ring-cyan-400">
        ...
        {/* Play Online */}
        <button onClick={onSwitchToLobby} className="p-6 bg-gray-800 rounded-lg shadow-lg text-left transition-transform transform hover:scale-105 hover:shadow-amber-500/30 focus:outline-none focus:ring-2 focus:ring-amber-400">
      ...
      <div className="mt-8 flex gap-4">
        <button onClick={() => navigate('/how-to-play')} className="text-gray-400 hover:text-white transition-colors">
          How to Play
        </button>
        <span className="text-gray-600">|</span>
        <button onClick={() => navigate('/leaderboard')} className="text-gray-400 hover:text-white transition-colors">
          Leaderboard
        </button>
      </div>
